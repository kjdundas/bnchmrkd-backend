...
export default function SnapshotStory({ results, formatTime, onAddPBs }) {
  ...
  const onSeg = (t) => {
    if (t.level === curLevel) return setRead({ tag: ['NOW', 'rgba(249,115,22,0.15)', O], msg: `${Her} level right now — ${t.name} (${fmt(t.time)}${unit}).` })
    const diff = +(isTime ? (pb - t.time) : (t.time - pb)).toFixed(2)
    if (diff > 0) setRead({ tag: ['TARGET', 'rgba(96,165,250,0.15)', '#60a5fa'], msg: `${t.name} needs ${fmt(t.time)}${unit} — ${she}'s ${Math.abs(diff)}${unit} away.` })
    else setRead({ tag: ['CLEARED', 'rgba(52,211,153,0.15)', '#34d399'], msg: `${t.name} needs ${fmt(t.time)}${unit} — ${she}'s already ${Math.abs(diff)}${unit} inside it.` })
  }
  ...
  return (
    <div ref={stage} className="landing-font" style={{ color: '#f5f5f7' }}>
      ...
      <div style={{ marginBottom: 6 }}>
        <h1 data-a="v" style={{ fontSize: 34, fontWeight: 700, letterSpacing: '-0.025em', lineHeight: 1.12 }}>
        ...
        <p data-a="v" style={{ color: '#9aa1ac', fontSize: 15, marginTop: 8 }}>Here's the read on {fmt(pb)}{unit}, in three steps.</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr auto 1fr', alignItems: 'stretch', margin: '24px 0 20px' }} className="ss-rail">
        <Beat cls="s1" accent="#94a3b8" step="Step 1 · Where she is">
          <div style={numS}>{fmt(pb)}<span style={numU}>{unit}</span></div>
        ...
        <Beat cls="s2" accent={O} step="Step 2 · How good that is">
          {pct != null ? (<>
            <Dots pct={pct} />
            <div style={{ fontSize: 32, fontWeight: 700 }}>{dotN}<span style={{ fontSize: 15, color: '#9aa1ac' }}> of 100</span></div>
        ...
        <Beat cls="s3" accent="#34d399" step="Step 3 · What's next">
          {gap != null ? (<>
            <div style={{ ...numS, color: '#34d399' }}>+{gap}<span style={numU}>{unit}</span></div>
            <div style={sayS}><b style={bS}>That's all that separates {her} from {nextName} level</b> — roughly one strong season.</div>
          </>) : (<><div style={{ ...numS, color: '#fbbf24' }}>Top</div><div style={sayS}>She's already at the ceiling tier for her age group.</div></>)}
        </Beat>
      </div>
      ...
      {pro && (
        <div className="ss-pro" style={{ position: 'relative', overflow: 'hidden', borderRadius: 18, padding: '28px 30px 24px', margin: '6px 0 18px', border: '1px solid rgba(249,115,22,0.3)', background: 'radial-gradient(120% 140% at 50% 0%, rgba(249,115,22,0.14) 0%, rgba(249,115,22,0.04) 38%, rgba(10,10,12,0) 72%), #101014', boxShadow: '0 30px 80px -40px rgba(249,115,22,0.5)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1.1fr', alignItems: 'center', gap: 18 }}>
            <div data-side="you" style={{ textAlign: 'right' }}>
              <div style={lblS}>HER · AT AGE {age}</div>
              <div style={{ fontSize: 54, fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>{fmt(pb)}<span style={{ fontSize: 22, color: '#9aa1ac' }}>{unit}</span></div>
            </div>
            <div data-side="mid" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
              ...
              <span className="mono-font" style={{ fontSize: 9.5, letterSpacing: '.1em', color: '#6b7280', textTransform: 'uppercase' }}>same speed · same age</span>
            </div>
            <div data-side="pro" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.13)', borderRadius: 14, padding: '16px 20px' }}>
              ...
              <div style={{ fontSize: 42, fontWeight: 800, color: O, letterSpacing: '-0.03em', fontVariantNumeric: 'tabular-nums' }}>{fmt(pro.timeAtSimilarAge)}<span style={{ fontSize: 18, color: '#9aa1ac' }}>{unit}</span></div>
            </div>
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, marginTop: 20, paddingTop: 16, borderTop: '1px solid rgba(255,255,255,0.07)', fontSize: 13, color: '#9aa1ac' }}>
            Went on to →
            ...
            <span style={chipS}>Career best {fmt(pro.pb)}{unit}</span>
      ...
      {ladder.length > 1 && (
        <div className="ss-ladder-card" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '18px 22px', marginBottom: 14 }}>
          ...
          <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 14 }}>Hover any level to see what it would take.</div>
          <div style={{ display: 'flex', height: 46, borderRadius: 9, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.13)' }}>
            {ladder.map((t, i) => {
              const f = ladder.length > 1 ? i / (ladder.length - 1) : 0
              const isYou = t.level === curLevel
              return (
                <div key={t.level} onMouseEnter={() => onSeg(t)} onMouseLeave={() => setRead(null)}
                  style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'DM Mono',monospace", fontSize: 9.5, color: f > 0.55 ? '#0a0a0b' : 'rgba(255,255,255,0.65)', background: heat(f), cursor: 'pointer', position: 'relative', outline: isYou ? '2px solid #fff' : 'none', outlineOffset: '-2px', zIndex: isYou ? 2 : 1 }}>
                  {t.name}
                  {isYou && <span className="mono-font" style={{ position: 'absolute', top: -22, left: '50%', transform: 'translateX(-50%)', whiteSpace: 'nowrap', fontSize: 9, color: '#0a0a0b', background: '#fff', padding: '2px 7px', borderRadius: 4 }}>HER · {fmt(pb)}</span>}
                </div>
              )
            })}
          </div>
